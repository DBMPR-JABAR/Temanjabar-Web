...
console.log(pembangunanTalikuatByUptd);
...
let categories = {
    UPTD: Object.keys(pembangunanTalikuatByUptd).map((key) => `UPTD ${key}`),
};
...
const seriesDrildownUptd = Object.keys(pembangunanTalikuatByUptd).map((key) => {
    categories[`UPTD-${key}`] = pembangunanTalikuatByUptd[key].map(
        (data) => data.nm_paket
    );
    return {
        name: `UPTD-${key}`,
        ...
        data: pembangunanTalikuatByUptd[key].map((data) => {
            const progres = dataTalikuat.find(
                (_data) => _data.id_data_umum == data.id
            );
            const persentase = Number(
                Number(progres ? progres.persentase : 0).toFixed(2)
            );
            return {
                name: data.nm_paket,
                y: persentase,
        ...
        tooltip: {
            ...
            pointFormat:
                '<span style="color:{point.color}">Progres <b>{point.y}%</b><br/>',
        },
    };
});
...
Object.keys(detailSeriesCurva).map((key) => {
    ...
    curvaDrilldown[`CURVA-${key}`] = {
        ...
        id: `CURVA-${key}`,
        color: "#00FF00",
        data: detailSeriesCurva[key].rencana.map((val) =>
...
Object.keys(detailSeriesCurva).map((key) => {
    curvaDrilldown[`CURVA-${key}-2`] = {
        name: `CURVA-${key}-2`,
        id: `CURVA-${key}-2`,
        data: detailSeriesCurva[key].realisasi.map((val) =>
            Number(Number(val).toFixed(2))
        ),
        type: "line",
        tooltip: {
        ...
        events: {
            drilldown: function (e) {
                ...
                if (!e.seriesOptions) {
                    ...
                    chart.xAxis[0].setCategories(
                        e.seriesOptions.data.map((data) => data.name)
                    );
            },
            drillup: function (e) {
