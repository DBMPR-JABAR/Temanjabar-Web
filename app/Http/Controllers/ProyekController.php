...
class ProyekController extends Controller
{
    ...
    public function getProyekKontrakAPI(Request $request)
    {
        $listProyekKontrak = DB::connection('dwh')->table('TBL_TALIKUAT_TRX_PROYEK_KONTRAK_PROGRESS_HARIAN as A')
            ...
            ->select(DB::raw('(SELECT MIN(TANGGAL)  FROM TBL_TALIKUAT_TRX_PROYEK_KONTRAK_PROGRESS_HARIAN WHERE NMP = A.NMP)  as DATE_FROM'),
                     DB::raw('(SELECT MAX(TANGGAL)  FROM TBL_TALIKUAT_TRX_PROYEK_KONTRAK_PROGRESS_HARIAN WHERE NMP = A.NMP ) as DATE_TO'),
                             'A.ID', 'A.NMP', 'A.TANGGAL', 'A.RENCANA_VOLUME_HaRIAN', 'A.RENCANA_VOLUME_KUMULATIF', 'A.REALISASI_VOLUME_HARIAN', 'A.REALISASI_VOLUME_KOMULATIF', 'A.PROGRESS_FISIK_RENCANA_BOBOT', 'A.PROGRESS_FISIK_RENCANA_KUMULATIF', 'A.PROGRESS_FISIK_REALISASI_BOBOT', 'A.PROGRESS_FISIK_REALISASI_KUMULATIF', 'A.DEVIASI_PROGRESS_FISIK', 'A.RENCANA_KEUANGAN_HARIAN', 'A.RENCANA_KEUANGAN_KOMULATIF','A.REALISASI_KEUANGAN_HARIAN','A.REALISASI_KEUANGAN_HARIAN1','B.UPTD','B.NAMA KEGIATAN as NAMA_KEGIATAN','B.PENYEDIA JASA as PENYEDIA_JASA');
        ...
        if ($request->kegiatan != "") $listProyekKontrak = $listProyekKontrak->where('NAMA_KEGIATAN', '=', $request->kegiatan);
            ...
            $ProyekKontrakData = [
                ...
                "data" => [
                    ...
                    [
                        ...
                        "completed" => [
                            "fill" => "#7CB5EC",
                            "amount" => (!empty($proyek->PROGRESS_FISIK_RENCANA_KUMULATIF) ? ($proyek->PROGRESS_FISIK_RENCANA_KUMULATIF *  100) : 0)
                        ]
                    ],
                    [
                        ...
                        "completed" => [
                            "fill" => "#7CB5EC",
                            "amount" => (!empty($proyek->PROGRESS_FISIK_REALISASI_KUMULATIF) ? ($proyek->PROGRESS_FISIK_REALISASI_KUMULATIF * 100) : 0)
                        ]
                    ],
                    [
