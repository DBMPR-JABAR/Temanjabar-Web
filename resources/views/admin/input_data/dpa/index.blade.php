@extends('admin.layout.index') @section('title') DPA @endsection
@section('head')
    ...
    <div class="col-lg-4">
        <div class="page-header-breadcrumb">
            <ul class="breadcrumb breadcrumb-title">
                ...
                <li class="breadcrumb-item"><a href="#!">DPA</a></li>
            </ul>
        </div>
    </div>
</div>
    ...
    <div class="col-sm-12">
        <div class="card">
            ...
            <div class="card-block">
                @if (hasAccess(Auth::user()->internal_role_id, 'DPA',
                'Create'))
                <a href="{{ route('dpa.create') }}" class="mb-3 btn btn-mat btn-primary">Tambah</a>
                @endif
                <div class="dt-responsive table-responsive">
                    <table id="dpa_table" class="table table-striped table-bordered able-responsive">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama Paket</th>
                                ...
                                <th>Pagu Anggaran DPA Pergeseran (Rp)</th>
                                <th>Check</th>
                                <th>Pendanaan</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $data->nama_uptd }}</td>
                                <td>{{ $data->tahun_anggaran }}</td>
                                <td>{{ $data->pagu_anggaran }}</td>
                                <td>{{ $data->pagu_anggaran_dpa_pergeseran }}</td>
                                <td>{{ $data->nama_check }}</td>
                                <td>{{ $data->nama_pendanaan }}</td>
                                <td>{{ $data->nama_pengadaan }}</td>
                                <td>{{ $data->kode_ruas_jalan }}</td>
                                <td style="min-width: 75px">
                                    <div class="btn-group" role="group" data-placement="top" title=""
                                        ...
                                        'DPA', 'Update'))
                                        <a class="d-inline-block" href="{{ route('dpa.edit', $data->no_urut) }}"><button
                                        ...
                                        @if(hasAccess(Auth::user()->internal_role_id,
                                        'DPA', 'Delete'))
                                        <a class="d-inline-block" href="#delModal" data-id="{{ $data->no_urut }}" data-toggle="modal"><button
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Hapus Data DPA</h4>
...
<script>
    ...
    $("#delModal").on("show.bs.modal", function (event) {
        ...
        console.log(id);
        const url = `{{ url('admin/input-data/dpa/delete') }}/` + id;
        console.log(url);
        const modal = $(this);
